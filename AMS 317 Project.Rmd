...
# Hypothesis 3: Do properties in certain neighborhoods receive significantly more reviews than others?
```{r}
...
# Boxplot of Reviews per Month by Neighborhood
ggplot(data, aes(x = neighbourhood, y = reviews_per_month)) +
  geom_boxplot(fill = "lightblue", color = "darkblue") +
  labs(
    title = "Reviews per Month by Neighborhood",
    x = "Neighborhood",
    y = "Reviews per Month"
  ) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), minimal())

# Kruskal-Wallis test for reviews per month by neighborhood
kruskal_test <- kruskal.test(reviews_per_month ~ neighbourhood, data = data)
kruskal_test

# Violin plot to show distribution in each neighborhood
ggplot(data, aes(x = neighbourhood, y = reviews_per_month)) +
